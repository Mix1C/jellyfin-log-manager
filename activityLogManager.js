(function () {
  ...
  function showModal({ icon, iconClass, title, body, inputPlaceholder, confirmText, confirmClass, onConfirm }) {
  ...
  async function clearActivityLog() {
    ...
    try {
      ...
      if (res.ok || res.status === 204) {
        ...
        showToast(`Failed (${res.status}): ${body.error || ''}`, 'error');
      }
  ...
  async function backupDatabase(savePath) {
    ...
    const serverUrl = getServerUrl();
    try {
      const res = await fetch(`${serverUrl}/System/Info`, { headers: { 'X-Emby-Token': token } });
      const info = await res.json();
      const dataPath = info.ProgramDataPath || '~/Library/Application Support/jellyfin/data';
      ...
      modal.innerHTML = `
        <div class="alm-modal" style="width:460px">
          ...
          <input class="alm-modal-input" value='cp "${dataPath}/jellyfin.db" "${savePath}"' readonly onclick="this.select()" style="cursor:text" />
          <div class="alm-modal-btns">
    ...
    } catch (e) { showToast('Could not get server info', 'error'); }
  }
  ...
  function injectToolbar() {
    ...
    waitForElement('.css-wsew38 [role="group"]', (tabGroup) => {
      ...
      toolbar.querySelector('#almRestartBtn').addEventListener('click', () => {
        showModal({ title: 'Restart Jellyfin', body: 'This will restart Jellyfin. The page will reload automatically after a few seconds.', confirmText: 'Restart', confirmClass: 'confirm-yellow', onConfirm: restartJellyfin });
      });
    });
  }
